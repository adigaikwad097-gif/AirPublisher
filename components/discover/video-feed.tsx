...
export function VideoFeed({ initialVideos = [], initialFilter = 'latest' }: VideoFeedProps) {
    const [videos, setVideos] = useState<Video[]>(initialVideos)
    const [filter, setFilter] = useState<FilterType>(initialFilter)
    const [loading, setLoading] = useState(false)
    const [hasMore, setHasMore] = useState(initialVideos.length >= 20 || initialVideos.length === 0)
    const [page, setPage] = useState(1)
    const observerTarget = useRef<HTMLDivElement>(null)

    const fetchVideos = async (currentFilter: FilterType, currentPage: number) => {
        const limit = 20
        const offset = (currentPage - 1) * limit

        // Instead of calling /api/discover, we do that logic here directly:
        const fetchedVideos = await getAllPostedVideos(limit, offset)
        ...
        let sortedVideos = [...videoWithCreators] as Video[]
        if (currentFilter === 'top' || currentFilter === 'trending') {
    ...
    // Load initial videos if none were provided
    useEffect(() => {
        if (initialVideos.length === 0 && !loading && page === 1) {
            const loadInitial = async () => {
                setLoading(true)
                const data = await fetchVideos(filter, 1)
                setVideos(data)
    ...
    const loadMore = useCallback(async () => {
        ...
        try {
            ...
            const newVideos = await fetchVideos(filter, nextPage)
    ...
    const handleFilterChange = async (newFilter: FilterType) => {
        ...
        try {
            const newVideos = await fetchVideos(newFilter, 1)
            setVideos(newVideos)
            setHasMore(newVideos.length >= 20)
        } catch (error) {
                            ...
                            <div className="p-4">
                                ...
                                <div className="flex items-center justify-between pt-3 border-t border-white/10">
                                    <div className="flex items-center gap-6">
                                        <VideoActions videoId={video.id} initialLikeCount={video.likes} />
                                    </div>
                                </div>
                            </div>
                        </div>
