...
function getStatusEmoji(entry: LeaderboardEntry): string {
  const revenue = entry.total_revenue || 0
  ...
  if (entry.rank <= 10) return '★' // Star - top 10
...
function getLast7dGrowth(entry: LeaderboardEntry, weeklyEntry?: LeaderboardEntry): number {
  ...
  return weeklyEntry.total_revenue || 0
}
...
export function LeaderboardContent({ allTimeEntries, weeklyEntries, currentCreatorId }: LeaderboardContentProps) {
  ...
  const sortedEntries = [...filteredEntries].sort((a, b) => {
    ...
    if (sortBy === 'revenue_views') {
      const ratioA = a.total_revenue && a.total_revenue > 0 ? a.total_views / a.total_revenue : 0
      const ratioB = b.total_revenue && b.total_revenue > 0 ? b.total_views / b.total_revenue : 0
      return ratioB - ratioA
    }
    return (b.airscore || 0) - (a.airscore || 0)
  })
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              <div className="px-4 py-2 bg-cyan-400/20 border border-cyan-400/30 rounded-full">
                <span className="text-cyan-300 font-semibold text-sm">
                  AIR: {topPlayer.airscore ? topPlayer.airscore.toFixed(3) : '—'}
                </span>
              </div>
              ...
              <div className="px-4 py-2 bg-purple-400/20 border border-purple-400/30 rounded-full">
                <span className="text-purple-300 font-semibold text-sm">
                  V/R: {topPlayer.total_revenue && topPlayer.total_revenue > 0 
                    ? (topPlayer.total_views / topPlayer.total_revenue).toFixed(1)
                    : '—'}
                </span>
              </div>
            </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <div className="px-3 py-1 bg-cyan-400/20 border border-cyan-400/30 rounded-full">
                      <span className="text-cyan-300 text-xs font-medium">
                        AIR: {entry.airscore ? entry.airscore.toFixed(3) : '—'}
                      </span>
                    </div>
                    ...
                    <div className="px-3 py-1 bg-purple-400/20 border border-purple-400/30 rounded-full">
                      <span className="text-purple-300 text-xs font-medium">
                        V/R: {entry.total_revenue && entry.total_revenue > 0
                          ? (entry.total_views / entry.total_revenue).toFixed(1)
                          : '—'}
                      </span>
                    </div>
